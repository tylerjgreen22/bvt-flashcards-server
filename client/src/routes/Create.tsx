...
const Create = () => {
  const { id } = useParams();
  const [existingSet, setExistingSet] = useState<FlashcardSet | null>(null);
  ...
  // If there is an ID (clone set), fetch that set and populate form
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      if (id) {
        const set = await agent.Set.detail(id);
        setExistingSet(set);
      }
      setLoading(false);
    };
  ...
  if (existingSet) {
    initialValues = {
      ...
      flashcards: existingSet.flashcards as Flashcard[],
    };
  }
  ...
  // If loading, return form loading skeleton
  if (loading) {
    return (
  ...
  return (
    <div className="max-w-[1100px] mx-auto mt-6 p-2 pb-52 md:pb-0">
      {/* Page title  */}
      <h2 className="text-black lg:text-white text-5xl mb-8">
        Create a New Deck
      </h2>
            ...
            <div className="flex flex-col gap-4 mb-6">
              ...
              <div>
                ...
                >
                  ...
                  <p>Add Set Description (Optional)</p>
                </button>
                <ErrorMessage
                  name="title"
                  component="div"
